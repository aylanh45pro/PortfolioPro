...
function initLanguageSelector() {
    ...
    if (savedLang && (savedLang === 'fr' || savedLang === 'en')) {
        ...
        updateProjectCards(); // Mettre à jour les cartes dès le chargement
    }
...
function updateTerminalCommands() {
    // Mettre à jour les commandes du terminal en fonction de la langue
    const terminalCommands = translations[currentLang].terminal_commands;
    
    // Relancer l'animation du terminal si elle existe
    if (typeof animateTerminal === 'function') {
        ...
        const commands = document.querySelectorAll('[id^="typed-command-"]');
...
function updateProjectCards() {
    ...
    projectCards.forEach((card, index) => {
        if (projectData[index]) {
            ...
            if (title && translations[currentLang][projectData[index].titleKey]) {
                title.textContent = translations[currentLang][projectData[index].titleKey];
            }
            
            if (category && translations[currentLang][projectData[index].categoryKey]) {
                category.textContent = translations[currentLang][projectData[index].categoryKey];
            }
            
            if (description && translations[currentLang][projectData[index].descriptionKey]) {
                description.innerHTML = translations[currentLang][projectData[index].descriptionKey];
            }
        }
    });
}
